...
func main() {
	...
	listen_socket, err := net.Listen("tcp", "127.0.0.1:8080")  //打开监听接口
	if err != nil {
...
func handle(conn net.Conn) {
	for {
		...
		switch msg_str[0] {
		case "nick":  //加入聊天室
			fmt.Println(msg_str[0],"   ",msg_str[1], "   ", msg_str[2])
			control.Login(msg_str[1], msg_str[2])
			fmt.Println(conn.RemoteAddr(), "-->", msg_str[1])  //nick占在数组下标0上，客户端上写的昵称占在数组下标1上
			for k, v := range ConnMap {  //遍历集合中存储的客户端消息
				if k != msg_str[1] {
					v.Write([]byte("[" + msg_str[1] + "]: 登录or注册成功"))
				}
			}
		...
		case "say":   //转发消息
			for k, v := range ConnMap {  //k指客户端昵称   v指客户端连接服务器端后的地址
				if k != msg_str[1] {  //判断是不是给自己发，如果不是
					...
					X,_ := strconv.Atoi(msg_str[2])
					Y,_ := strconv.Atoi(msg_str[3])
					fmt.Println(X, Y)
					i := game.Game(X, Y)
					fmt.Println(game.Qi)
					fmt.Println(i)
					i0 := strconv.Itoa(i)
					v.Write([]byte("[" + msg_str[1] + "]:|" + msg_str[2] + "|" + msg_str[3] + "|" + i0) )  //给除了自己的每一个客户端发送自己之前要发送的消息
				}
			}
		case "quit":  //退出
